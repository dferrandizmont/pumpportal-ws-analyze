 ...
 */
function calculateCreatorStats(creatorTracking) {
  ...
  const averageSellPrice = sellHistory.length > 0
    ? sellHistory.reduce((sum, sell) => sum + (sell.price || 0), 0) / sellHistory.length
    : 0;
 ...
 */
function generateActiveAlerts(creators, threshold) {
  return creators
    .map(creator => {
