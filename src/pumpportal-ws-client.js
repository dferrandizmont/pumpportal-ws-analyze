...
class PumpPortalWSClient {
  ...
  connect() {
    ...
    try {
      const wsUrl = config.pumpPortal.apiKey
        ? `${config.pumpPortal.wsUrl}?api-key=${config.pumpPortal.apiKey}`
  ...
  handleReconnection() {
    ...
    const delay = config.app.reconnectDelayMs * Math.pow(1.5, this.reconnectAttempts - 1); // Exponential backoff
...
module.exports = PumpPortalWSClient;
